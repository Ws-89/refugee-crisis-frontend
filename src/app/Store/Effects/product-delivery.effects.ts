...
import { getProductDeliveryList, getProductDeliveryListSuccess, addProductDelivery, addProductDeliverySuccess, deleteProductDelivery, deleteProductDeliverySuccess, updateProductDeliverySuccess, updateProductDelivery } from "../Actions/product-delivery.action";
...
export class ProductDeliveryEffects {
    ...
    loadProductDeliveries$ = createEffect(() => 
        this.action$.pipe(
        ofType(getProductDeliveryList),
        exhaustMap(() => this.productDeliveryService.getProductDeliveryList().pipe(
            map((productDeliveries) => getProductDeliveryListSuccess(productDeliveries)),
            catchError(() => EMPTY)
        )
    )));
